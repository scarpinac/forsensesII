...
class StoreRequest extends FormRequest
{
    ...
    {
        return auth()->user()->canAccess('cadastro.cliente.store');
    }
    ...
    public function rules(): array
    {
        ...
        if ($this->isPessoaFisica()) {
            $rules = array_merge($rules, [
                'cpf' => 'required|string|formato_cpf|cpf|unique:cliente,cpf',
        ...
        if ($this->isPessoaJuridica()) {
            $rules = array_merge($rules, [
                'cnpj' => 'required|string|formato_cnpj|cnpj|unique:cliente,cnpj',
