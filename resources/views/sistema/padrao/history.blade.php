...
@endsection
@section('title', __('labels.padrao.breadcrumb.history') )

@section('content_header')
    <div class="container-fluid">
        <div class="row align-items-center mb-2">
            <div class="col-sm-6">
                <h1 class="m-0">{{ __('labels.padrao.breadcrumb.history') }} - {{$padrao->descricao}}</h1>
            </div>
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right mb-0">
                    ...
                    <li class="breadcrumb-item active">{{ __('labels.padrao.breadcrumb.history') }}</li>
                </ol>
            </div>
        </div>
    </div>
...
<div class="card">
    <div class="card-header">
        <h3 class="card-title">{{ __('labels.padrao.history.data.title') }}</h3>
...
<div class="card">
    <div class="card-header">
        <h3 class="card-title">{{ __('labels.padrao.history.changes.title') }}</h3>
    </div>
    <!-- /.card-header -->
    <div class="card-body">
        <div class="table-responsive">
            <table class="table-system table-bordered table-hover">
                ...
                <tbody>
                    @forelse ($padrao->historicos as $historico)
                        <tr>
                            ...
                            <td class="whiteSpace-nowrap text-center">
                                <button type="button" class="detalhes btn btn-outline-info btn-sm"
                                    ...
                                    <i class="fas fa-search-plus"></i> {{ __('labels.padrao.history.button.details') }}
                                </button>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" class="text-center">{{ __('labels.padrao.no.history') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="detailsModalLabel">{{ __('labels.padrao.modal.details.title') }}</h5>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">{{ __('labels.padrao.modal.close') }}</button>
            </div>
        </div>
    </div>
</div>
