...
@section('title', __('labels.revenda.title.edit') )

@section('content_header')
    <div class="container-fluid">
        <div class="row align-items-center mb-2">
            <div class="col-sm-6">
                <h1 class="m-0">{{ __('labels.revenda.title.edit') }} - {{ $revenda->nomeFantasia }}</h1>
            </div>
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right mb-0">
                    ...
                    <li class="breadcrumb-item active">{{ __('labels.revenda.title.edit') }}</li>
                </ol>
            </div>
        </div>
    </div>
...
@section('content')
    <div class="card">
        <div class="card-body">
            <form action="{{ URL::signedRoute('cadastro.revenda.update', $revenda->id) }}" method="POST">
                @method('PUT')
                @include('cadastro.revenda.form')
                <div class="d-flex justify-content-between">
                    <button type="submit" class="btn btn-success"><i class="fas fa-save"></i> {{ __('labels.revenda.save_changes') }}</button>
