...
@endsection
@section('title', __('labels.price_table.title.create') )
@section('content_header')
    <div class="container-fluid">
        <div class="row align-items-center mb-2">
            <div class="col-sm-6">
                <h1 class="m-0">{{ __('labels.price_table.title.create') }}</h1>
            </div>
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right mb-0">
                    ...
                    <li class="breadcrumb-item active">{{ __('labels.price_table.title.create') }}</li>
                </ol>
            </div>
        </div>
    </div>
...
@section('content')
    <div class="card">
        <div class="card-body">
            <form action="{{ URL::signedRoute('cadastro.tabela_preco.store') }}" method="POST">
                @include('cadastro.tabela_preco.form')
                <div class="d-flex justify-content-between">
                    <button type="submit" class="btn btn-primary"><i class="fas fa-save"></i> {{ __('labels.price_table.save') }}</button>
